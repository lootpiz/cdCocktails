...
if(length(combo_idx) > 0) {
	...
	Z <- array(as.numeric(as.matrix(combo_mat[order(combo_mat$concentrationA, combo_mat$concentrationB), 6])),
		dim=c(length(combo_B_concentration), length(combo_A_concentration)))
	colnames(Z) <- c(combo_A_concentration)
	rownames(Z) <- c(combo_B_concentration)
	Z
	...
	write.table(t(result), file=paste0("tbl.decrease.combo_summary_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")
	write.table(raw_matrix, file=paste0("tbl.decrease.raw_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")
	write.table(bliss_matrix, file=paste0("tbl.decrease.bliss_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")
	write.table(loewe_matrix, file=paste0("tbl.decrease.loewe_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")
	write.table(hsa_matrix, file=paste0("tbl.decrease.hsa_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")
	write.table(zip_matrix, file=paste0("tbl.decrease.zip_", idSample, "__", idDrugA, "__", idDrugB, ".txt"),
		row.names=F, col.names=F, quote=F, sep="\t")

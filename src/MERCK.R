...
mono_ncol <- max(count.fields("../data/MERCK/merck_mono.txt"))
mono <- read.table("../data/MERCK/merck_mono.txt", header=F, row.names=1, sep="\t", fill=TRUE, stringsAsFactor=F, col.names=paste0('V', seq_len(mono_ncol)))

combo <- read.table("../data/MERCK/merck_combo.txt", header=T, row.names=1, sep="\t", fill=TRUE, stringsAsFactor=F)
	...
	for(drug_idx_x2 in c(1:length(drug_names))) {
		...
		if (drug_name_X1 != drug_name_X2) {
			...
			if(length(target_idx) > 0) {
				...
				combo_mat_z <- as.matrix(combo_mat[order(combo_mat$concentrationB, combo_mat$concentrationA), 8])
...
write.table(outfile_stats, file="../results/tbl.merck.combo_summary.txt", row.names=F, col.names=F, quote=F, sep="\t")

write.table(raw_matrix, file="../results/tbl.merck.raw.txt", row.names=F, col.names=T, quote=F, sep="\t")
write.table(bliss_matrix, file="../results/tbl.merck.bliss.txt", row.names=F, col.names=T, quote=F, sep="\t")
write.table(loewe_matrix, file="../results/tbl.merck.loewe.txt", row.names=F, col.names=T, quote=F, sep="\t")
write.table(hsa_matrix, file="../results/tbl.merck.hsa.txt", row.names=F, col.names=T, quote=F, sep="\t")
write.table(zip_matrix, file="../results/tbl.merck.zip.txt", row.names=F, col.names=T, quote=F, sep="\t")
